...
library(readxl)
library(stringr)
sce <- readH5AD("/maiziezhou_lab2/yuling/datasets/Develop/5DPIs.h5ad")
##############
library(SingleCellExperiment)
library(zellkonverter)
library(Seurat)
...
# ##### reference 
old_names <- Cells(total_data)
batch_vec  <- total_data$Batch

new_names <- paste0(old_names, "_", batch_vec)

# check duplication
any(duplicated(new_names))
total_data <- RenameCells(total_data, new.names = new_names)
...
#df <- read.csv("/maiziezhou_lab2/yuling/MouseSpinal/label_transfer/RCTD/regeneration_ref/counts.csv", header = TRUE, check.names = FALSE)
