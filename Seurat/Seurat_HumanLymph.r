...
predictions <- TransferData(
  anchorset = anchors,
  refdata = ref@meta.data$original_clusters,
  dims = 1:30
)

# Add predictions to query metadata
# raw <- AddMetaData(raw, metadata = predictions)
write.csv(predictions, file = "/maiziezhou_lab2/yuling/MouseSpinal/label_transfer/Seurat/HumanLymph_output/seurat_HumanLymph.csv")
